...
let AuthService = class AuthService {
    ...
    async resendOTPCode(email) {
        const user = await this.userModel.findOne({ email });
        if (!user)
            throw new common_1.NotFoundException('user not fdound');
        const otpCode = Math.random().toString().slice(2, 8);
        const validationDate = new Date();
        validationDate.setTime(validationDate.getTime() + 3 * 60 * 1000);
        await this.userModel.updateOne({ _id: user._id }, {
    ...
    async continueWithGoogle({ fullName, email, avatar }) {
        ...
        if (!existUser)
            existUser = await this.userModel.create({ email, fullName, avatar, isActive: true });
        const payload = {
            id: existUser._id
        };
        const token = this.jwtService.sign(payload, { expiresIn: '1h' });
    ...
    async getCurrentUser(userId) {
        ...
        if (!user)
            throw new common_1.NotFoundException('user not dfound');
